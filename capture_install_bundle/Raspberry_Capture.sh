#!/bin/bash
# Stratux Serial Capture Script
# Background capture with monitoring and clean shutdown
# Captures data from /dev/ttyUSB0 into files in /home/pi/

SERIAL_PORT="/dev/ttyUSB0"
BAUD_RATE=115200
CAPTURE_DIR="/home/pi"
CONFIG_FILE="$CAPTURE_DIR/stream_counter.conf"
LOG_FILE="$CAPTURE_DIR/capture_monitor.log"
...
get_output_filename() {
    ...
    printf "$CAPTURE_DIR/stream_%04d.txt" "$COUNTER"
}
...
check_already_running() {
    if [ -f "$PID_FILE" ]; then
        ...
        if kill -0 "$old_pid" 2>/dev/null; then
            log_message "INFO" "Capture already running (PID $old_pid)"
            exit 0
        else
            rm -f "$PID_FILE"
        fi
...
log_message "INFO" "Output directory: $CAPTURE_DIR"

# Write PID file
echo $$ > "$PID_FILE"

# Run capture (not daemonized - systemd manages the process)
run_capture
